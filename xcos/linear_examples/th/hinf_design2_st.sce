...
Agp=[Ap zeros(size(Ap,1),size(Ad,2)) zeros(size(Ap,1),size(Awp,2)) zeros(size(Ap,1),size(Awt,2));
    ...
    Bwt*Cp zeros(size(Awt,1),size(Ad,2)) zeros(size(Awt,1),size(Awp,2))  Awt];
Bgp = [zeros(size(Bp,1),size(Bd,2)) zeros(size(Bp,1),size(Bwp,2)) Bp; 
        ...
        zeros(size(Bwt,1),size(Bd,2)) zeros(size(Bwt,1),size(Bwp,2)) zeros(size(Bwt,1),size(Bp,2))];
...
[Ak,Bk,Ck,Dk]=hinf(Agp,Bgp,Cgp,Dgp,1,1,10000);
