// HINF_DESIGN_ST.SCE   perform H-inf design on servo robot joint   
// cast weights on S and T
...
s=poly(0,'s');
// real plant created with commands from chapter 6
//pqvnum = r2d*ka1*ka2*km1*km2*(tau*s+1+ka2*km1)*...
//((1-jm*wn^2)*s^2+2*z*wn*s+wn^2); 
//pqvden = s*((jm*s+bm)*(tau*s+1+ka2*km1)^2+ ... 
//(tau*s+1)*km1*km2*ke)*(s^2+2*z*wn*s+wn^2); 
//Pqv = syslin('c',pqvnum/pqvden)
...
//pqxden = ((jm*s+bm)*(tau*s+1+ka2*km1)^2+(tau*s+1)*km1*km2*ke)*(s^3+2*z*wn*s^2+wn^2*s);
pqx = syslin('c',r2d*pqxnum/pqxden);
Pqv = pqx*pxv;

pd = pqx;
// plant model obtained from experiment. Parameter errors are random within +/- 5% of real
// values
...
//pqvnum_a = r2d*ka1_a*ka2_a*km1_a*km2_a*(tau_a*s+1+ka2_a*km1_a)*...
//((1-jm_a*wn_a^2)*s^2+2*z*wn_a*s+wn_a^2); 
//pqvden_a = (s+0.001)*((jm_a*s+bm_a)*(tau_a*s+1+ka2_a*km1_a)^2+ ... 
//(tau_a*s+1)*km1_a*km2_a*ke_a)*(s^2+2*z*wn_a*s+wn_a^2); 
//Pqv_a = syslin('c',pqvnum_a/pqvden_a)
//
...
sw1=0;sw2=0;  // switch counts
for k=1:fsize, // correct phase wrap at -180
    pquph(k)=real(pquph(k));
    pquph_a(k)=real(pquph_a(k));
    aph1=real(pquph(k));
    aph2=real(pquph_a(k));
    if aph1>0 then sw1=1; end
    if (aph2>0) then sw2=1; end
    if sw1==1    
        pquph(k)=pquph(k)-360;
    end
        
    if sw2==1 then
        pquph_a(k)=pquph_a(k)-360;
    end
end
figure(1);
subplot(211),plot2d("ln",frq,[20*log10(pqumag) 20*log10(pqumag_a)]);
ylabel('Magnitude (dB)');
subplot(212),plot2d("ln",frq,[pquph pquph_a]);
ylabel('Phase (degree)');
xlabel('Frequency (Hz)');
...
Agp=[Ap zeros(size(Ap,1),size(Awp,2)) zeros(size(Ap,1),size(Awt,2));
    ...
    Bwt*Cp zeros(size(Awt,1),size(Awp,2)) Awt];
Bgp = [zeros(size(Bp,1),size(Bwp,2)) Bp; 
        Bwp zeros(size(Bwp,1),size(Bp,2));
        zeros(size(Bwt,1),size(Bwp,2)) zeros(size(Bwt,1),size(Bp,2))];
...
//
Cgp = [-Dwp*Cp Cwp zeros(size(Cwp,1),size(Cwt,2));
        Dwt*Cp zeros(size(Cwt,1),size(Cwp,2)) Cwt;
        -Cp zeros(size(Cp,1),size(Cwp,2)) zeros(size(Cp,1),size(Cwt,2))];
Dgp = [Dwp 0.001; 0 0;1 0];  // makes D12 full rank
