//rne_dh.sci Compute inverse dynamics via recursive Newton-Euler formulation
//
// Recursive Newton-Euler for standard Denavit-Hartenberg notation.  Is invoked by
//rne().
//
//
// This file is adapted from The Robotics Toolbox for Matlab (RTB).
// Copyright (C) 1993-2011, by Peter I. Corke
// 

function [tau,wbase] = rne_dh(robot, a1, a2, a3, a4, a5)
    nargin = argn(2);
    z0 = [0;0;1];
    grav = robot.gravity;   // default gravity from the object
    fext = zeros(6, 1);
    n = robot.nj;    // number of joints

    // check that robot object has dynamic parameters for each link
    for j=1:n
        L = robot.Link(j);
        if isempty(L.r) | isempty(L.I) | isempty(L.m)
            error('dynamic parameters (m, r, I) not set in link %d', j);
        end
    end

    // Set debug to:
    //   0 no messages
    //   1 display results of forward and backward recursions
    //   2 display print R and p*
    debugflag = 0;

    if numcols(a1) == 3*n
        Q = a1(:,1:n);
        Qd = a1(:,n+1:2*n);
        Qdd = a1(:,2*n+1:3*n);
        np = numrows(Q);
        if nargin >= 3, 
            grav = a2(:);
        end
        if nargin == 4
            fext = a3;
        end
    else
        np = numrows(a1);
        Q = a1;
        Qd = a2;
        Qdd = a3;
        if numcols(a1) ~= n | numcols(Qd) ~= n | numcols(Qdd) ~= n | ...
            numrows(Qd) ~= np | numrows(Qdd) ~= np
            error('bad arguments');
        end
        if nargin >= 5, 
            grav = a4(:);
        end
        if nargin == 6
            fext = a5;
        end
    end
    
    tau = zeros(np,n);

    for p=1:np
        q = Q(p,:)';
        qd = Qd(p,:)';
        qdd = Qdd(p,:)';
    
        Fm = [];
        Nm = [];
        pstarm = [];
        Rm = [];
        w = zeros(3,1);
        wd = zeros(3,1);
        vd = grav(:);

    //
    // init some variables, compute the link rotation matrices
    //
        for j=1:n
            L = robot.Link(j);
            Tj = Link2AT(L,q(j));
            //Tj = link.A(q(j));
            if L.RP == 'R'
                d = L.d;
            else
                d = q(j);
            end
            alpha = L.alpha;
            pstar = [L.a; d*sin(alpha); d*cos(alpha)];
            if j == 1
                pstar = t2r(robot.base) * pstar;
                Tj = robot.base * Tj;
            end
            pstarm(:,j) = pstar;
            Rm(:,:,j) = t2r(Tj);
            if debugflag >1
                Rm(:,:,j)
                pstarm(:,j)'
            end
        end    // for j=1:n

    //
    //  the forward recursion
    //
    //pause;
        for j=1:n
            L = robot.Link(j);

            Rt = Rm(:,:,j)';    // transpose!!
            pstar = pstarm(:,j);
            r = L.r;

            //
            // statement order is important here
            //
            //pause;
            if L.RP == 'R'
                // revolute axis
                wd = Rt*(wd + z0*qdd(j) + ...
                    cross(w,z0*qd(j)));
                w = Rt*(w + z0*qd(j));
                //v = cross(w,pstar) + Rt*v;
                vd = cross(wd,pstar) + ...
                    cross(w, cross(w,pstar)) +Rt*vd;

            else
                // prismatic axis
                w = Rt*w;
                wd = Rt*wd;
                vd = Rt*(z0*qdd(j)+vd) + ...
                    cross(wd,pstar) + ...
                    2*cross(w,Rt*z0*qd(j)) +...
                    cross(w, cross(w,pstar));
            end

            //whos
            vhat = cross(wd,r') + ...
                cross(w,cross(w,r')) + vd;
            F = L.m*vhat;
            N = L.I*wd + cross(w,L.I*w);
            Fm = [Fm F];
            Nm = [Nm N];
            
            if debugflag
                printf('w: '); printf('%.3f ', w)
                printf('\nwd: '); printf('%.3f ', wd)
                printf('\nvd: '); printf('%.3f ', vd)
                printf('\nvdbar: '); printf('%.3f ', vhat)
                printf('\n');
            end
        end    // for j=1:n

    //
    //  the backward recursion
    //

        fext = fext(:);
        f = fext(1:3);      // force/moments on end of arm
        nn = fext(4:6);

        for j=n:-1:1
            L = robot.Link(j);
            pstar = pstarm(:,j);
            
            //
            // order of these statements is important, since both
            // nn and f are functions of previous f.
            //
            if j == n
                R = eye(3,3);
            else
                R = Rm(:,:,j+1);
            end
            r = L.r;
            nn = R*(nn + cross(R'*pstar,f)) + ...
                cross(pstar+r',Fm(:,j)) + ...
                Nm(:,j);
            f = R*f + Fm(:,j);
            if debugflag
                printf('f: '); printf('%.3f ', f)
                printf('\nn: '); printf('%.3f ', nn)
                printf('\n');
            end

            R = Rm(:,:,j);
            if L.RP == 'R'
                // revolute
                tau(p,j) = nn'*(R'*z0) + ...
                    L.G^2 * L.Jm*qdd(j) + ...
                    lfriction(L,qd(j));
            else
                // prismatic
                tau(p,j) = f'*(R'*z0) + ...
                    L.G^2 * L.Jm*qdd(j) + ...
                    lfriction(L, qd(j));
            end    // if L.RP == 'R'
            //pause;
        end    // for j = n:-1:1
        // this last bit needs work/testing
        R = Rm(:,:,1);
        nn = R*(nn);
        f = R*f;
        
        wbase = clean([f; nn]);
    end    // for p = 1:np
    tau = clean(tau);
endfunction