//CPOLY Generate scalar polynomial trajectory using cubic polynomial
//
// [S,SD,SDD] = CPOLY(S0, SF, M) is a scalar trajectory (Mx1) that varies 
...
//
// [S,SD,SDD] = CPOLY(S0, SF, T) as above but specifies the trajectory in 
...
function [s,sd,sdd] = cpolyplot(q0, qf, t, qd0, qdf)
    t0 = t;
    nargin=argn(2);
    nargout = argn(1);
    if isscalar(t)
